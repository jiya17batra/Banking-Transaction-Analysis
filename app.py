...
# ============================================================
# SIDEBAR FILTERS (apply to ALL tabs, including Overview)
# ============================================================
...
if fdf.empty:
    st.warning("No transactions match the current filters. Try widening your selection in the sidebar.")
    st.stop()
...
""")
    ...
    st.caption("These numbers reflect the current sidebar filters.")
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Transactions (filtered)", f"{len(fdf):,}", help=f"out of {len(df):,} total")
    c2.metric("Customers (filtered)", f"{fdf['customer_id'].nunique():,}", help=f"out of {df['customer_id'].nunique():,} total")
    c3.metric("Fraud Rate", f"{100*fdf['is_fraud'].mean():.2f}%")
    c4.metric("ML Model ROC-AUC", "0.975", help="Full-dataset model metric (does not change with filters)")

    col1, col2 = st.columns(2)
    with col1:
        st.markdown("**Transaction frequency heatmap (day of week × hour)**")
        st.caption("Reference chart from the full dataset")
    ...
    st.info("Use the **Trends**, **Customer Behavior**, and **Fraud Detection** tabs "
            "for fully interactive, filtered charts.")
